...
export function ArticleToc({ items, mobile = false }: { items: TocItem[]; mobile?: boolean }) {
    ...
    <ul className="border-l border-outline-variant">
      {items.map((item) => (
        <li key={`${item.id}-${item.label}`}>
          <a href={`#${item.id}`} className={`block border-l px-3 py-1.5 font-mono text-xs uppercase transition-colors focus-visible:animate-flicker focus-visible:bg-primary-container focus-visible:text-on-primary-container ${active === item.id ? "-ml-px border-primary-fixed-dim bg-primary-muted/10 text-primary-fixed-dim" : "border-transparent text-on-surface-variant hover:border-primary-dim hover:text-on-surface"}`}>
            {item.label}
          </a>
        </li>
      ))}
  ...
  return <aside className="hidden lg:sticky lg:top-24 lg:block lg:self-start" aria-label="Table of contents"><h2 className="mb-3 font-mono text-[11px] uppercase tracking-[0.1em] text-primary-fixed-dim">ON_THIS_PAGE</h2>{links}</aside>;
}
